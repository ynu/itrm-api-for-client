...
[
  generalUser,
  generalUser2,
].map(user => describe(`websites controller, user: ${user.id}`, () => {
  let agent;
  before(async () => {
  ...
  it('GET 获取列表，仅能获取自己具有权限的', (done) => {
    agent.get(controller).query({
      ...
      expect(res).to.have.header('Content-Range', 'websites 0-1/3');
      done();
    });
  });
...
describe('未登录用户：websites controller', () => {
  let agent;
  before(() => {
    agent = chai.request.agent(app);
  });
