...
import { list, totalCount, getById, updateById, deleteById, insert, listFilter } from '../middlewares/wechat-official-accounts';
  ...
  router.get('/',
    ...
    formatQuery({
      success: (queryOptions, req, res, next) => {
        info('wechat-official-accounts list queryOptions:', queryOptions);
  ...
  router.post('/',
    ...
    insertChangeLog({
      ...
      getData: req => ({
        ...
        resource: {
          category: resources.WECHAT_OFFICIAL_ACCOUNTS,
    ...
    (req, res, next) => {
      ...
      if (isAdmin(roles)) next();
        ...
        try {
          ...
          error('wechat-official-accounts getOneCheck:', err.message);
